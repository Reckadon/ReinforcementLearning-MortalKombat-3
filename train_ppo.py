...
class ppoTrainer:
    ...
    def load_checkpoint(self,from_latest=True):
        """Load model checkpoint if available."""
        
        # Find all checkpoint files
        checkpoint_pattern = os.path.join(self.config["save_dir"], "checkpoint_episode_*.pth")
        ...
        if from_latest:
            if checkpoints:
                ...
                for cp in checkpoints:
                    try:
                        ep_str = os.path.basename(cp).split('_')[-1].split('.')[0]
                        episodes.append(int(ep_str))
                    except ValueError:
                        continue
                if episodes:
                    latest_episode = max(episodes)
                    checkpoint_path = os.path.join(self.config["save_dir"], f"checkpoint_episode_{latest_episode}.pth")
            ...
            print("Checkpoint loading skipped, starting from scratch")

        
    def save_checkpoint(self):
        """Save model checkpoint with the episode number."""
        os.makedirs(self.config["save_dir"], exist_ok=True)
        checkpoint_path = os.path.join(
            self.config["save_dir"], f"checkpoint_episode_{self.episode_count}.pth"
        )
        torch.save({
    ...
    def rollout(self, n_steps):
        ...
        Returns:
            dict: Rollout data
        """
        print("Starting rollout...")  # Log rollout start
        observations = []
        frames_list = []
        features_list = []
        actions = []
        ...
        obs, _ = self.env.reset()
        print("Environment reset at rollout start")  # Log environment reset
        frame_raw = obs.get("frame")
        if frame_raw is None:
            print("WARNING: env returned empty frame — resetting env")
            obs, _ = self.env.reset()
            frame_raw = obs["frame"]

        frame = preprocess_frame(frame_raw).to(self.device)
        features = extract_features(obs).to(self.device)
        self.frame_stack.reset(frame)

        for step in range(n_steps):
            # print(f"Rollout step {step + 1}/{n_steps}")  # Log each step
            ...
            if done:
                print("Environment done, resetting...")  # Log environment reset
                obs, _ = self.env.reset()
                frame_raw = obs.get("frame")
                if frame_raw is None:
                    print("WARNING: env returned empty frame — resetting env")
        ...
        print("Rollout completed")  # Log rollout completion
        return {
    ...
    def update(self, rollout_data):
        """
        PPO update: clipped surrogate objective + multi-epoch minibatch updates.
        """
        ...
        # Convert everything to tensors on device
        ...
        minibatch_size = self.config.get("minibatch_size", 32)
        ...
        for epoch in range(update_epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                policy_loss = -torch.min(
                    ratio * advantages[mb_idx],
                    clipped_ratio * advantages[mb_idx]
                ).mean()
        ...
        # Logging (PPO logs last minibatch)
        self.writer.add_scalar("Losses/Policy", policy_loss.item(), self.step_count)
        self.writer.add_scalar("Losses/Value", value_loss.item(), self.step_count)
        self.writer.add_scalar("Losses/Entropy", entropy_loss.item(), self.step_count)
        self.writer.add_scalar("Losses/Total", loss.item(), self.step_count)
    ...
    def train(self):
        ...
        try:
            while self.episode_count < self.config["max_episodes"]:
                print(f"Starting Episode {self.episode_count}")  # Log episode start

                # Collect rollout
                rollout_data = self.rollout(self.config["n_steps"])
                print(f"Rollout collected for Episode {self.episode_count}")  # Log rollout completion

                # Update model
                loss_info = self.update(rollout_data)
                print(f"Model updated for Episode {self.episode_count}")  # Log update completion
                ...
                print(f"Ending Episode {self.episode_count}")  # Log episode end
                self.episode_count += 1

        except KeyboardInterrupt:
            print("\nTraining interrupted by user")
        except Exception as e:
            print(f"Error during training: {e}")  # Log any exceptions
        finally:
            self.save_checkpoint()
            self.writer.close()
            self.env.close()
